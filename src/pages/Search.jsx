...
function Search() {
 ...
 setOrder(par);
  ...
  <button
    ...
    className="border-2 p-1 px-3 rounded-md shadow-md w-[6vw] active:scale-90 transition-transform duration-150 ease-in-out"
  >
    Filter
  </button>

  {isOpen && (
    <ul className="absolute w-[230%] right-5 mt-3 z-10 rounded-lg flex flex-col shadow-xl bg-white border-[1px] py-1 transition-all duration-200 origin-top transform opacity-100 scale-100">
      <li
        onClick={() => handleOrderBy("asc")}
        className="p-1 hover:bg-[#F3F4F6] cursor-pointer"
      >
        Price: Low to High
      </li>
      <li
      ...
      <div className="grid  w-full h-full grid-cols-4  ">
        {finalData?.map((item, index) => (
          <div
            key={index}
            className="rounded-lg mb-0    m-auto w-[88%] h-[92%]  group relative cursor-pointer flex items-center justify-center overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30"
          >
            ...
            <div className="absolute inset-0 flex translate-y-[56%] text-white flex-col items-center justify-center px-9 text-center transition-all duration-500 group-hover:translate-y-0">
              ...
              <button
                ...
                className="hover:scale-110 ease-out duration-300 rounded-full  mb-4 bg-[#3f4142a4]   py-2 px-3.5 font-com text-sm capitalize text-white shadow shadow-black/60"
              >
                see more
              </button>
              <h3>${item?.price}</h3>
      ...
      {isFetchingNextPage && (
        <div className="w-full flex items-center justify-center ">
          {" "}
          <span className="loading loading-dots   mt-2 text-[#00BADB]  loading-xl mb-2"></span>
        </div>
      )}
    </div>
  );
